...
mod tests {
    ...
    #[test]
    fn test_1385_additional() {
        // edge cases
        assert_eq!(Solution::find_the_distance_value(vec![], vec![1, 2, 3], 5), 0);
        assert_eq!(Solution::find_the_distance_value(vec![1, 2, 3], vec![], 0), 3);
        assert_eq!(Solution::find_the_distance_value(vec![0,0,0], vec![0], 0), 0);
        assert_eq!(Solution::find_the_distance_value(vec![0,0,0], vec![1], 0), 3);
    }
}
