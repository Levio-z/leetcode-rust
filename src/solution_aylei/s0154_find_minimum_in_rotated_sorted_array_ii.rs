...
// 针对无重复的做法, 只要二分搜索找折点即可: 假如 nums[mid] > nums[base]
// 那么转折点一定在右侧, 否则在左侧
//
// 但假如有重复, 就可能有 nums[mid] == nums[base], 这时就尴尬了,
// 无法确定转折点在左半部分还是右半部分
//
// 可以考虑一个数组, [1,1,1,1,1,1,1,0,1,1,1,1,1,1] 这个数组无论怎么去找 0,
// 时间复杂度无法低于 O(N)
//
// 但假如不是这种极端情况, 那么二分搜索还是能优化的, 在 153 的基础上,
// 碰到相等就跳过即可
impl Solution {
