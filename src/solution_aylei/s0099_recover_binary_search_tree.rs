...
// - 两侧都合法: 递归对左右子树进行上述操作
//
// 直接使用上述思路会重复迭代子树, 因此可以先用一次中序遍历, 将值的分布记录在数组中, 之后可以用
// O(1) 的时间得到某个 node 两侧的值分布
//
// 这种办法的时间空间复杂度都是 O(N)
//
...
// - 假如 node 合法,
//   那么只需要把这个子树中的最大值和最小值返回到上一个层级来帮助判断上一个层级的子树是否合法即可,
//   无需记录这个子树的所有值
// - 假如 node 不合法, 按最开始的办法进行处理, 除非...
// - 只有一侧值不合法, 这时候由于是 Bottom-up, 不能直接交换不合法的值与 root,
//   而要判断交换后能否合法(Top-down 的办法中由于题目给定了有且仅有 swap 一次, 因此与 root
//   交换必然是合法的)
//
