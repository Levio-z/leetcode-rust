...
//   |      |              *         =
//          v           y     -1  0      -x
// n-y,x <- n-x,n-y  if we consider axis transform, then: rotate(x, y) = (y, -x
// + n)

//  we only need to iterate a 1/4 corner matrix, for odd matrix, we take an
// extra part in x direction
//
//  even:
//
//  x x o o
