...
// 典型的动态规划, 我们求以 node_i 为 root 的最大和, 可以下推到求 root 的左右子树, 这里要注意,
// 路径是不能分叉的, 因此 我们记 f[node] 为以 node 为根的最大和, 记 g[node] 为 node 为根,
// *最多连通一侧子树*的最大和
//
...
//
// 显然, g[None] = 0 (None 即空子树), 最终计算到 g[root] 中止, f
// 的最大值会在计算过程中出现(注意 f[root] 不一定是最大值)
//
