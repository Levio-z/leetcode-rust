...
// 思路: DFS, 三个指针 i,j,k 分别指向 s1, s2, s3 已经消费到的 char 位置,
// 下一个可以走的路径是 s3 当前消费到的 char 值
//
// 如 aaaaaas     aaaaaaaw    aaaaaaaaaaaaaasw
// 那么第一步可以从 s1 或 s2 取一个 char, 用 DFS 的方式搜索整个解空间
//
// 优化: 直接 DFS 非常慢, 还是上面的例子, 最差情况是大量重复字符,
// 时间复杂度直接是 2^(M+N), 优化方式借鉴 DP 经常用到的
// memorize, 使用一个二维数组缓存每一对遍历过的 i,j 最后是否能产生合法的
// interleaving.
//
// 优化后通过缓存剪除的路径比较难分析, 但很显然能知道最差情况也只需要将所有 M*N
// 的组合进行标记, 因此最差时间复杂度 O(M*N) 空间复杂度 O(M*N)
